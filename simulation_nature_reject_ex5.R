...
## same as ex4, but here we don't have the same number of samples with an exposure of 1 for each signature
...
## if the signature definitions are good in that they are probabilities
if( all(colSums(f1[,-ncol(f1)]) == 1) & all(colSums(f2[,-ncol(f2)]) == 1) ){
  ...
  stop('Check true signature definitions')
}
